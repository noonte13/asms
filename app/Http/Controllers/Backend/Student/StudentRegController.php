...
class StudentRegController extends Controller
{
    ...
    public function StudentRegStore(Request $request){
    	...
    	$student = User::where('roll','Student')->orderBy('id','DESC')->first();
     ...
     $student = User::where('roll','Student')->orderBy('id','DESC')->first()->id;
     	$studentId = $student+1;
     	if ($studentId < 10) {
    			$id_no = '000'.$studentId;
    		}elseif ($studentId < 100) {
    	...
    	if ($request->file('picture')) {
    		...
    		$file->move(public_path('upload/student_pictures'),$filename);
 ...
 public function StudentRegUpdate(Request $request,$student_id){
    	...
    	if ($request->file('picture')) {
    		$file = $request->file('picture');
    		@unlink(public_path('upload/student_pictures/'.$user->image));
    		$filename = date('YmdHi').$file->getClientOriginalName();
    		$file->move(public_path('upload/student_pictures'),$filename);
    		$user['picture'] = $filename;
    	}
 ...
 public function StudentUpdatePromotion(Request $request,$student_id){
    	...
    	if ($request->file('picture')) {
    		$file = $request->file('picture');
    		@unlink(public_path('upload/student_pictures/'.$user->picture));
    		$filename = date('YmdHi').$file->getClientOriginalName();
    		$file->move(public_path('upload/student_pictures'),$filename);
    		$user['picture'] = $filename;
    	}
