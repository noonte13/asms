...
class FeeController extends Controller
{
  ...
  $data = AssignStudent::with(['discount'])->where('year_id',$year_id)->where('class_id',$class_id)->get();
    	 ...
    	 $html['thsource'] .= '<th>Date of Birth</th>';
...
if($accountstudentfees !=null) {
 	...
 	$html[$key]['tdsource']  .= '<td>'.$std['student']['dob']. '<input type="hidden" name="class_id" value= " '.$std->class_id.' " >'.'</td>';

 	$html[$key]['tdsource']  .= '<td>'.$registrationfee->amount.'MVR'.'<input type="hidden" name="date" value= " '.$date.' " >'.'</td>';
    ...
    public function StudentFeeStore(Request $request){
    	...
    	return redirect()->route('fee.view')->with($notification);
    		...
    		$notification = array(
    		'message' => 'Sorry, Could not update data',
    		'alert-type' => 'error'
    	);
